...
class BenGeS(Dataset):
    def __init__(
        self, data_index, root_dir, number_of_classes, wandb, bands="RGB", transform=None, normalization_value=10_000
    ):
    ...
    def __getitem__(self, idx):
        ...
        lulc_vector = np.where(label_vector <= threshold, 0, label_vector)
        label_vector = np.squeeze(lulc_vector)
