...
def get_n_weighted_samples(df, number_of_samples):
    ...
    used_ids = []
    for c in df.columns:
        if c in ['filename', 'patch_id', 'snow_and_ice', 'mangroves', 'moss_and_lichen']:
            continue
...
def create_split_ben_ge_s():
    ...
    df_validation.to_csv(data_index_path_to_store + 'ben-ge-s-validation.csv', index=False)
...
def create_split_ben_ge_m():
    ...
    df_validation.to_csv(data_index_path_to_store + 'ben-ge-m-validation.csv', index=False)
...
def create_split_ben_ge_l():
    ...
    df_validation.to_csv(data_index_path_to_store + 'ben-ge-validation.csv', index=False)
