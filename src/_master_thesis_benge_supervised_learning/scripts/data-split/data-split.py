...
def make_split():
    ...
    # Load ben-ge
    df_large = pd.read_csv('/ds2/remote_sensing/ben-ge/ben-ge/esaworldcover/npy/ben-ge_esaworldcover.csv')

    # Load ben-ge-s
    df_small_train = pd.read_csv('/ds2/remote_sensing/ben-ge/ben-ge-s/data-index/ben-ge-s-train.csv')
    df_small_validation = pd.read_csv('/ds2/remote_sensing/ben-ge/ben-ge-s/data-index/ben-ge-s-validation.csv')
    df_small_test = pd.read_csv('/ds2/remote_sensing/ben-ge/ben-ge-s/data-index/ben-ge-s-test.csv')
    ...
    for key, value in dict_size.items():
        ...
        for index, row in df_large.iterrows():
            ...
            if not_present and len(df_validation) < len_validation_and_test:
                df_validation = pd.concat([df_validation, row.to_frame().T], ignore_index=True)
                continue
        ...
        df_validation.to_csv(data_path + 'ben-ge-validation'+key+'.csv', index=False)
